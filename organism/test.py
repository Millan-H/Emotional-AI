...
class EnvClassifier:
    ...
    def getObjs(self, camData, rangeOfVisionPx):
        ...
        print(objs)
        return edges
    def getObjsColorBlob(self, camData, rangeOfVisionPx):
        image=cv2.cvtColor(np.array(camData)[0], cv2.COLOR_BGR2HSV)
        colorRangeSet=[]
        camDataSpliced=[]
        for i in range(len(camData)):
            if i%rangeOfVisionPx==0:
                camDataSpliced.append([])
                spliceCount+=1
            camDataSpliced[spliceCount].append(camDataSpliced[i])
        
        for i in range(len(camDataSpliced)):
            for j in range(len(camDataSpliced[i])):
                if len(colorRangeSet)==0:
                    colorRangeSet.append([[camDataSpliced[i][j][0]-4,camDataSpliced[i][j][1]-4,camDataSpliced[i][j][2]-4],[camDataSpliced[i][j][0]+camDataSpliced[i][j][1]+4,camDataSpliced[i][j][2]+4]])
                else:
                    for k in range(len(colorRangeSet)):
                        if not(colorRangeSet[k][0][0]<camDataSpliced[i][j][0]<colorRangeSet[k][1][0] and colorRangeSet[k][1][1]<camDataSpliced[i][j][1]<colorRangeSet[k][1][1] and colorRangeSet[k][0][2]<camDataSpliced[i][j][2]<colorRangeSet[k][0][2]):
                            colorRangeSet.append([[camDataSpliced[i][j][0]-4,camDataSpliced[i][j][1]-4,camDataSpliced[i][j][2]-4],[camDataSpliced[i][j][0]+camDataSpliced[i][j][1]+4,camDataSpliced[i][j][2]+4]])
        
        masks=[]
        for i in range(len(colorRangeSet)):
            masks.append(cv2.inRange(image, colorRangeSet[i][0], colorRangeSet[i][1]))
